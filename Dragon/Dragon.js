...
(function (Dragon) {
    ...
    Dragon.pointAppendMatrix = pointAppendMatrix;
    ...
    Dragon.invertMatrix = invertMatrix;
    ...
    Dragon.matrixAppendMatrix = matrixAppendMatrix;
    var PI = Math.PI;
    var HalfPI = PI / 2;
    var PacPI = PI + HalfPI;
    var TwoPI = PI * 2;
...
(function (Dragon) {
    var TickerListener = (function () {
        function TickerListener(name, ticker_listener) {
            this.name = name;
            this.ticker_listener = ticker_listener;
        }
        return TickerListener;
    }());
    Dragon.TickerListener = TickerListener;
    var Ticker = (function () {
        function Ticker() {
        ...
        Ticker.prototype.unregister = function (listener) {
            ...
            //     if (listener.name == this.listeners[i].name) {
            //         for (var j = i; j < this.listeners.length; j++) {
        ...
        Ticker.prototype.notify = function (deltaTime) {
            for (var _i = 0, _a = this.listeners; _i < _a.length; _i++) {
                var listener = _a[_i];
                //listener(deltaTime);
                listener.ticker_listener(deltaTime);
            }
        };
        return Ticker;
    }());
...
(function (Dragon) {
    ...
    Dragon.TouchEventListener = TouchEventListener;
    ...
    Dragon.TouchEventListenerManagement = TouchEventListenerManagement;
})(Dragon || (Dragon = {}));
var Dragon;
(function (Dragon) {
    var DisplayObject = (function () {
        function DisplayObject(type) {
            ...
            this.type = type;
            this.relativeMatrix = new Dragon.Matrix();
            this.overallMatrix = new Dragon.Matrix();
        }
        DisplayObject.prototype.update = function () {
            ...
            if (this.parent) {
                ...
                this.overallMatrix = Dragon.matrixAppendMatrix(this.relativeMatrix, this.parent.overallMatrix);
            }
            else {
        ...
        DisplayObject.prototype.addEventListener = function (type, func, capture) {
            var listener = new Dragon.TouchEventListener(type, func, capture);
            ...
            for (var i = 0; i < this.eventListenerList.length; i++) {
                if (this.eventListenerList[i].type == type && this.touchEnabled == true) {
                    if (this.eventListenerList[i].capture == true) {
                        Dragon.TouchEventListenerManagement.list.unshift(this.eventListenerList[i]);
                    }
                    else if (this.eventListenerList[i].capture == false) {
                        Dragon.TouchEventListenerManagement.list.push(this.eventListenerList[i]);
                    }
                }
            }
        };
    ...
    Dragon.DisplayObject = DisplayObject;
    var DisplayObjectContainer = (function (_super) {
        ...
        DisplayObjectContainer.prototype.hitTest = function (x, y) {
            for (var i = this.list.length - 1; i >= 0; i--) {
                ...
                var invertChildRelativeMatrix = Dragon.invertMatrix(this.relativeMatrix);
                var resultPoint = Dragon.pointAppendMatrix(point, invertChildRelativeMatrix);
    ...
    Dragon.DisplayObjectContainer = DisplayObjectContainer;
    var TextField = (function (_super) {
        ...
        TextField.prototype.hitTest = function (x, y) {
            ...
            var invertChildRelativeMatrix = Dragon.invertMatrix(this.relativeMatrix);
            var resultPoint = Dragon.pointAppendMatrix(point, invertChildRelativeMatrix);
            var rectangle = new Dragon.Rectangle(0, 0, this.text.length * 10, this.fontSize);
    ...
    var Bitmap = (function (_super) {
        ...
        Bitmap.prototype.hitTest = function (x, y) {
            ...
            var invertChildRelativeMatrix = Dragon.invertMatrix(this.relativeMatrix);
            var resultPoint = Dragon.pointAppendMatrix(point, invertChildRelativeMatrix);
            var rectangle = new Dragon.Rectangle(0, 0, this.image.width, this.image.height);
            if (rectangle.isPointInRectangle(resultPoint)) {
                // console.log(this);
                return this;
            }
    ...
    var MovieClip = (function (_super) {
        ...
        function MovieClip(data) {
            ...
            this.tickerListener = new Dragon.TickerListener(this.data._name, this.ticker);
            this.isRegist = false;
            //    this.play();
        }
        MovieClip.prototype.play = function () {
            if (this.isRegist == false) {
                Dragon.Ticker.getInstance().register(this.tickerListener);
            }
            this.isRegist = true;
        };
        MovieClip.prototype.stop = function () {
            Dragon.Ticker.getInstance().unregister(this.tickerListener);
            this.isRegist = false;
        };
    ...
    Dragon.MovieClip = MovieClip;
})(Dragon || (Dragon = {}));
var Dragon;
(function (Dragon) {
    Dragon.run = function (canvas) {
        var stage = new Dragon.DisplayObjectContainer();
        ...
        var frameHandler = function () {
            ...
            Dragon.Ticker.getInstance().notify(deltaTime);
        ...
        var doMouseEvent = function (e, type) {
            ...
            var result = target;
            if (result) {
                result.dispatchEvent(type);
                while (result.parent) {
                    var currentTarget = result.parent;
                    //    let e = { type, target, currentTarget };
                    result.parent.dispatchEvent(type);
                    result = result.parent;
                }
                Dragon.TouchEventListenerManagement.dispatch(e);
            }
        };
